...
function InputFieldEmail({ text, inputplaceholder, onChange, error, type ,handleVerifyClick , verified }) {

    return (
        <div>
            <h1 className="px-5 pb-1 text-sm text-left text-grey font-semibold">{text}</h1>
            ...
            {error && <p className="italic text-red-500 text-xs mt-1 px-5">{error}</p>}
        </div>
    );
}
